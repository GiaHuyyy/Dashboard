...
export function FormActions({ onSave, onSaveMail, onSaveStay, onReset, isSubmitting, isUploading }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
      <Button icon={Save} label="Lưu" onClick={onSave} disabled={isDisabled} variant="primary" />
      <Button icon={Save} label="Lưu gửi mail" onClick={onSaveMail} disabled={isDisabled} variant="info" />
      <Button icon={Save} label="Lưu tại trang" onClick={onSaveStay} disabled={isDisabled} variant="success" />
