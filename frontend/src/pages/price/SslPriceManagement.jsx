...
function SslPriceManagement() {
  ...
  const fetchRows = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await sslPriceApi.list({ search: searchText.trim() });
      const nextRows = Array.isArray(response?.sslPrices) ? response.sslPrices : [];
      setRows(nextRows);
      setSelectedIds((prev) => prev.filter((id) => nextRows.some((item) => item.id === id)));
    } catch (error) {
      toast.error(error?.message || "Không thể tải bảng giá SSL");
    } finally {
      setIsLoading(false);
    }
  }, [searchText]);
  ...
  const openCreate = () => navigate("/bang-gia/ssl/them-moi");
  const openEdit = (row) => navigate(`/bang-gia/ssl/chinh-sua/${row.id}`);
  ...
  const handleDelete = async () => {
    try {
      if (deleteRow?.id) {
        await sslPriceApi.remove(deleteRow.id);
        setRows((prev) => prev.filter((item) => item.id !== deleteRow.id));
        toast.success("Đã xóa bảng giá SSL");
      } else if (selectedIds.length > 0) {
        const response = await sslPriceApi.removeMany(selectedIds);
        setRows((prev) => prev.filter((item) => !selectedIds.includes(item.id)));
        toast.success(`Đã xóa ${response?.deletedCount || selectedIds.length} bảng giá SSL`);
      } else {
        const response = await sslPriceApi.removeMany([]);
        setRows([]);
        toast.success(`Đã xóa toàn bộ (${response?.deletedCount || 0}) bảng giá SSL`);
      }
    } catch (error) {
